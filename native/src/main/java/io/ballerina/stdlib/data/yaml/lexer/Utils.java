...
public class Utils {
    ...
    public static Pattern PRINTABLE_PATTERN = new PrintablePattern();
    public static Pattern JSON_PATTERN = new JsonPattern();
    public static Pattern BOM_PATTERN = new BomPattern();
    public static Pattern LINE_BREAK_PATTERN = new LineBreakPattern();
    public static Pattern WHITE_SPACE_PATTERN = new WhiteSpacePattern();
    public static Pattern DECIMAL_PATTERN = new DecimalPattern();
    public static Pattern HEXA_DECIMAL_PATTERN = new HexaDecimalPattern();
    public static Pattern WORD_PATTERN = new WordPattern();
    public static Pattern FLOW_INDICATOR_PATTERN = new FlowIndicatorPattern();
    public static Pattern INDICATOR_PATTERN = new IndicatorPattern();
    public static Pattern URI_PATTERN = new UriPattern();

    public static boolean matchPattern(YamlLexer.StateMachine sm, List<Pattern> inclusionPatterns) {
    ...
    public static boolean isPlainSafe(YamlLexer.StateMachine sm) {
    ...
    public static boolean isTagChar(YamlLexer.StateMachine sm) {
        return matchPattern(sm, List.of(URI_PATTERN, WORD_PATTERN, new CharPattern('%')),
                List.of(new CharPattern('!'), FLOW_INDICATOR_PATTERN));
    }

    public static boolean checkCharacters(YamlLexer.StateMachine sm, List<Character> expectedChars) {
        return checkCharacters(sm, expectedChars, -1);
    }

    public static boolean checkCharacters(YamlLexer.StateMachine sm, List<Character> expectedChars, int column) {
        if (column == -1) {
            column = sm.getColumn();
        }
    ...
    public static boolean isComment(YamlLexer.StateMachine sm) {
        return sm.peek() == '#' && (WHITE_SPACE_PATTERN.pattern(sm.peek(-1)) || sm.peek(-1) == -1);
    }

    public static boolean isMarker(YamlLexer.StateMachine sm, boolean directive) {
    ...
    public static boolean discernPlanarFromIndicator(YamlLexer.StateMachine sm) {
    ...
    public static String getWhitespace(YamlLexer.StateMachine sm) {
        String whitespace = "";
        while (sm.getColumn() < sm.dataBufferSize()) {
            if (sm.peek() == ' ') {
                whitespace += " ";
            } else if (sm.peek() == '\t') {
